...
func (c *SensorCommand) getSensors(cred *BrokerCredentials, service string) map[string]interface{} {
	guid, err := c.cliConnection.CliCommandWithoutTerminalOutput("service", service, "--guid")
	url := CreateRestCallUrlString(c.cliConnection, cred, "sensors/" + guid[0])
...
func (c *SensorCommand) isServiceReady(cred *BrokerCredentials, service string) bool {
	guid, err := c.cliConnection.CliCommandWithoutTerminalOutput("service", service, "--guid")
	url := CreateRestCallUrlString(c.cliConnection, cred, "is-running/" + guid[0])
	req, err := http.NewRequest("GET", url, nil)
	AssertErrorIsNil(err)
	body, _ := SendRequest(req)
...
func (c *SensorCommand) listSensors(cred *BrokerCredentials, service string) {
